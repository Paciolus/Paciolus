...
def _summary_to_response(summary: DiagnosticSummary) -> DiagnosticSummaryResponse:
    ...
    return DiagnosticSummaryResponse(
        ...
        current_assets=float(summary.current_assets),
        inventory=float(summary.inventory),
        total_liabilities=float(summary.total_liabilities),
        current_liabilities=float(summary.current_liabilities),
        total_equity=float(summary.total_equity),
        total_revenue=float(summary.total_revenue),
        cost_of_goods_sold=float(summary.cost_of_goods_sold),
        total_expenses=float(summary.total_expenses),
        operating_expenses=float(summary.operating_expenses or 0.0),
        ...
        materiality_threshold=float(summary.materiality_threshold),
