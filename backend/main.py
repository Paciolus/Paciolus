...
class BenchmarkComparisonRequest(BaseModel):
    """Request to compare client ratios to benchmarks."""
...
class BenchmarkSourcesResponse(BaseModel):
    """Response containing benchmark source information."""
...
@app.get("/benchmarks/industries", response_model=list[str])
async def get_benchmark_industries():
    """
    Get list of industries with available benchmark data.

    Public endpoint - no authentication required.
    Benchmark data is reference data, not client data.
    """
    industries = get_available_industries()
    return [ind.value for ind in industries]


@app.get("/benchmarks/sources", response_model=BenchmarkSourcesResponse)
async def get_benchmarks_sources():
    """
    Get benchmark data source attribution information.

    Public endpoint - no authentication required.
    Returns information about data sources, coverage, and disclaimers.
    """
...
@app.get("/benchmarks/{industry}", response_model=BenchmarkSetResponse)
async def get_industry_benchmarks(
    ...
    Public endpoint - no authentication required.
    Benchmark data is reference data (publicly available aggregate statistics).

    Returns percentile distributions (p10, p25, p50, p75, p90) for each ratio,
    along with mean, standard deviation, and sample size.
    """
...
@app.post("/benchmarks/compare", response_model=BenchmarkComparisonResponse)
...
async def compare_to_benchmarks(
    ...
    Client ratio data is ephemeral (computed in real-time, never stored).

    Zero-Storage Compliance:
    - Client ratios provided in request are not persisted
    - Comparison results are computed in real-time
    - Only benchmark reference data is used (public aggregate statistics)

    Returns percentile rankings, position labels, and interpretations
    for each provided ratio.
    """
    log_secure_operation(
        "benchmark_compare",
...
# =============================================================================
# PRIOR PERIOD COMPARISON ENDPOINTS (Sprint 51)
# =============================================================================
...
class PeriodSaveRequest(BaseModel):
    ...
    period_type: Optional[str] = Field(None, description="Period type: monthly, quarterly, annual")
    # Category totals
    total_assets: float = 0.0
    current_assets: float = 0.0
    inventory: float = 0.0
...
class CompareRequest(BaseModel):
    ...
    # Current period data (same structure as PeriodSaveRequest)
    total_assets: float = 0.0
    current_assets: float = 0.0
    inventory: float = 0.0
    total_liabilities: float = 0.0
...
@app.post("/clients/{client_id}/periods")
async def save_prior_period(
    ...
    ZERO-STORAGE COMPLIANCE:
    - Only aggregate totals are stored (no raw transaction data)
    - Period snapshots enable variance analysis without storing source files
    """
    ...
    # Parse period type
    parsed_period_type = None
    if period_data.period_type:
        try:
            parsed_period_type = PeriodType(period_data.period_type)
        except ValueError:
            raise HTTPException(status_code=400, detail="Invalid period_type. Use: monthly, quarterly, annual")

    # Create DiagnosticSummary record
    db_summary = DiagnosticSummary(
        client_id=client_id,
        user_id=current_user.id,
...
@app.get("/clients/{client_id}/periods", response_model=List[PeriodListItem])
async def list_prior_periods(
    ...
    Returns periods ordered by period_date (most recent first),
    with timestamp as secondary sort.
    """
    ...
    # Query periods with period_label (Sprint 51 added this field)
    periods = db.query(DiagnosticSummary).filter(
        DiagnosticSummary.client_id == client_id,
        DiagnosticSummary.user_id == current_user.id,
        DiagnosticSummary.period_label.isnot(None)  # Only periods with labels
    ).order_by(
...
async def compare_to_prior_period(
    ...
    Returns side-by-side comparison with variances for:
    ...
    ZERO-STORAGE COMPLIANCE:
    - Comparison results are computed on-demand (ephemeral)
    - Only aggregate totals are compared
    """
    ...
    # Build prior data dictionary from DiagnosticSummary
...
# Sprint 63: Three-way comparison + CSV export
...
@app.post("/audit/compare-three-way")
async def compare_three_way_trial_balances(
    ...
    Sprint 63: Three-way comparison endpoint.

    ZERO-STORAGE COMPLIANCE:
    - All three datasets processed in-memory only
    - Comparison results are ephemeral (never stored)
    """
    log_secure_operation(
...
async def export_csv_movements(
    ...
    Sprint 63: CSV export for multi-period comparison.

    ZERO-STORAGE COMPLIANCE:
    - CSV generated in-memory, streamed to user, never stored.
    """
    log_secure_operation(
