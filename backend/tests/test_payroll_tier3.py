...
class TestGhostEmployeeIndicators:
    ...
    def test_single_entry(self, detection_no_dept):
        ...
        entries = [
            PayrollEntry(
                employee_id="E001", employee_name="OneTime", pay_date=date(2025, 3, 15), gross_pay=50000, _row_index=1
            ),
        ]
        config = PayrollTestingConfig(ghost_min_indicators=1)
    ...
    def test_boundary_months_only(self, detection_no_dept):
        ...
        entries = [
            PayrollEntry(
                employee_id="E001", employee_name="Boundary", pay_date=date(2025, 1, 15), gross_pay=5000, _row_index=1
            ),
            PayrollEntry(
                employee_id="E001", employee_name="Boundary", pay_date=date(2025, 1, 30), gross_pay=5000, _row_index=2
            ),
            PayrollEntry(
                employee_id="E001", employee_name="Boundary", pay_date=date(2025, 6, 15), gross_pay=5000, _row_index=3
            ),
            # Non-boundary employees to establish period
            PayrollEntry(
                employee_id="E002", employee_name="Normal", pay_date=date(2025, 3, 15), gross_pay=5000, _row_index=4
            ),
        ]
        config = PayrollTestingConfig(ghost_min_indicators=1)
        result = _test_ghost_employee_indicators(entries, config, detection_no_dept)
...
class TestScoringCalibration:
    ...
    def test_clean_lower_than_moderate(self):
        ...
        clean_entries = [
            PayrollEntry(
                ...
                department="Sales",
                pay_date=date(2025, 3, i % 28 + 1),
                gross_pay=5000.0 + i * 10,
                _row_index=i,
            )
            for i in range(1, 51)
        ]
    ...
    def test_moderate_lower_than_dirty(self):
        ...
        # Dirty: many issues
        dirty_entries = [PayrollEntry(employee_name="", gross_pay=0, _row_index=i) for i in range(1, 11)]
        dirty_results = [
    ...
    def test_risk_tier_ordering(self):
        ...
        dirty_entries = [PayrollEntry(employee_name="", gross_pay=0, _row_index=i) for i in range(1, 6)]
        ...
        assert dirty_score.risk_tier in ("moderate", "elevated", "high")
