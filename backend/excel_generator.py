...
class PaciolusWorkpaperGenerator:
    ...
    def _build_summary_tab(self):
        ...
        # Legal disclaimer
        ws['A20'] = "DISCLAIMER: This output is generated by an automated analytical system and supports"
        ws['A21'] = "internal evaluation and professional judgment. It does not constitute an audit, review,"
        ws['A22'] = "or attestation engagement and provides no assurance."
        for row in [20, 21, 22]:
    ...
    def _build_anomalies_tab(self):
        ...
        # Headers
        headers = ["Account", "Type", "Issue", "Amount", "Severity", "Classification"]
        for col, header in enumerate(headers, start=1):
        ...
        if material:
            ...
            for ab in material:
                ws.cell(row=row, column=1, value=ab.get('account', 'Unknown'))
                ws.cell(row=row, column=2, value=ab.get('type', 'Unknown'))
                ws.cell(row=row, column=3, value=ab.get('issue', ''))
                ws.cell(row=row, column=3).alignment = Alignment(wrap_text=True)
                ws.cell(row=row, column=4, value=ab.get('amount', 0))
                ws.cell(row=row, column=4).style = 'currency_style'
                ws.cell(row=row, column=5, value=ab.get('severity', 'unknown').upper())
                ws.cell(row=row, column=6, value="MATERIAL")
                ws.cell(row=row, column=6).font = Font(bold=True, color=ExcelColors.CLAY)
        ...
        if immaterial:
            ...
            for ab in immaterial:
                ...
                ws.cell(row=row, column=4).style = 'currency_style'
                ws.cell(row=row, column=5, value=ab.get('severity', 'unknown').upper())
                ws.cell(row=row, column=6, value="IMMATERIAL")
                ws.cell(row=row, column=6).font = Font(color=ExcelColors.OBSIDIAN_500)
