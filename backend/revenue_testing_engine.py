"""Backward-compatibility shim for the revenue_testing_engine module.

The implementation moved to ``services.audit.revenue_testing.analysis``
...
``from revenue_testing_engine import ...`` statements continue to work.

Why dynamic re-export instead of explicit ``__all__``: the canonical
analysis module exposes 35+ public symbols plus several
underscore-prefixed test helpers that ``tests/test_revenue_testing*.py``
imports directly. Listing each by hand would invite drift; the
namespace copy below stays in sync automatically.

New code should import from ``services.audit.revenue_testing.analysis``.
"""

import sys as _sys

from services.audit.revenue_testing import analysis as _impl
