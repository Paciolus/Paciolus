...
# Benchmark percentile band thresholds (internal use only — NOT a risk score)
BENCHMARK_PERCENTILE_UPPER = 65
BENCHMARK_PERCENTILE_LOWER = 40
...
def generate_interpretation(ratio_name: str, percentile: int, industry: Industry, direction: RatioDirection) -> str:
    """
...
def compare_to_benchmark(ratio_name: str, client_value: float, benchmark: IndustryBenchmark) -> BenchmarkComparison:
    """
...
def compare_ratios_to_benchmarks(ratios: dict[str, float], benchmark_set: BenchmarkSet) -> list[BenchmarkComparison]:
    """
...
def get_percentile_band(score: float) -> str:
    """
    Get benchmark percentile band label from composite percentile.

    Args:
        score: Overall percentile score (0-100)

    Returns:
        Percentile band: "upper_quartile", "mid_range", "lower_quartile"
    """
    if score >= BENCHMARK_PERCENTILE_UPPER:
        return "upper_quartile"
    elif score >= BENCHMARK_PERCENTILE_LOWER:
        return "mid_range"
    else:
...
RETAIL_BENCHMARKS: dict[str, IndustryBenchmark] = {
    ...
    "operating_margin": _create_benchmark(
        "operating_margin", Industry.RETAIL, 0.02, 0.04, 0.06, 0.09, 0.14, mean=0.065, std_dev=0.038, sample_size=1250
    ),
    "roa": _create_benchmark(
        "roa", Industry.RETAIL, 0.02, 0.04, 0.06, 0.10, 0.15, mean=0.068, std_dev=0.042, sample_size=1250
    ),
    "roe": _create_benchmark(
        "roe", Industry.RETAIL, 0.05, 0.10, 0.15, 0.22, 0.35, mean=0.165, std_dev=0.095, sample_size=1250
    ),
...
MANUFACTURING_BENCHMARKS: dict[str, IndustryBenchmark] = {
    ...
    "quick_ratio": _create_benchmark(
        "quick_ratio", Industry.MANUFACTURING, 0.55, 0.85, 1.15, 1.55, 2.10, mean=1.22, std_dev=0.52, sample_size=980
    ),
    "debt_to_equity": _create_benchmark(
        "debt_to_equity",
        Industry.MANUFACTURING,
    ...
    "gross_margin": _create_benchmark(
        "gross_margin", Industry.MANUFACTURING, 0.15, 0.22, 0.30, 0.40, 0.52, mean=0.31, std_dev=0.11, sample_size=980
    ),
    "net_profit_margin": _create_benchmark(
        "net_profit_margin",
        Industry.MANUFACTURING,
    ...
    "roa": _create_benchmark(
        "roa", Industry.MANUFACTURING, 0.03, 0.05, 0.08, 0.12, 0.18, mean=0.085, std_dev=0.048, sample_size=980
    ),
    "roe": _create_benchmark(
        "roe", Industry.MANUFACTURING, 0.06, 0.10, 0.14, 0.20, 0.28, mean=0.148, std_dev=0.072, sample_size=980
    ),
    "inventory_turnover": _create_benchmark(
        "inventory_turnover",
        Industry.MANUFACTURING,
...
PROFESSIONAL_SERVICES_BENCHMARKS: dict[str, IndustryBenchmark] = {
    ...
    "roe": _create_benchmark(
        "roe", Industry.PROFESSIONAL_SERVICES, 0.12, 0.20, 0.28, 0.40, 0.55, mean=0.30, std_dev=0.14, sample_size=650
    ),
    "dso": _create_benchmark(
        "dso",
        Industry.PROFESSIONAL_SERVICES,
...
TECHNOLOGY_BENCHMARKS: dict[str, IndustryBenchmark] = {
    ...
    "quick_ratio": _create_benchmark(
        "quick_ratio", Industry.TECHNOLOGY, 1.25, 1.75, 2.45, 3.40, 4.90, mean=2.68, std_dev=1.18, sample_size=820
    ),
    "debt_to_equity": _create_benchmark(
        "debt_to_equity",
        Industry.TECHNOLOGY,
    ...
    "roa": _create_benchmark(
        "roa", Industry.TECHNOLOGY, 0.02, 0.06, 0.12, 0.20, 0.30, mean=0.135, std_dev=0.088, sample_size=820
    ),
    "roe": _create_benchmark(
        "roe", Industry.TECHNOLOGY, 0.05, 0.12, 0.20, 0.32, 0.48, mean=0.22, std_dev=0.135, sample_size=820
    ),
    "dso": _create_benchmark(
        "dso",
        Industry.TECHNOLOGY,
...
HEALTHCARE_BENCHMARKS: dict[str, IndustryBenchmark] = {
    ...
    "quick_ratio": _create_benchmark(
        "quick_ratio", Industry.HEALTHCARE, 0.85, 1.20, 1.65, 2.20, 3.00, mean=1.75, std_dev=0.70, sample_size=720
    ),
    "debt_to_equity": _create_benchmark(
        "debt_to_equity",
        Industry.HEALTHCARE,
    ...
    "gross_margin": _create_benchmark(
        "gross_margin", Industry.HEALTHCARE, 0.25, 0.35, 0.45, 0.58, 0.72, mean=0.46, std_dev=0.14, sample_size=720
    ),
    "net_profit_margin": _create_benchmark(
        "net_profit_margin",
        Industry.HEALTHCARE,
    ...
    "roa": _create_benchmark(
        "roa", Industry.HEALTHCARE, 0.02, 0.04, 0.07, 0.11, 0.16, mean=0.075, std_dev=0.045, sample_size=720
    ),
    "roe": _create_benchmark(
        "roe", Industry.HEALTHCARE, 0.05, 0.10, 0.15, 0.22, 0.32, mean=0.162, std_dev=0.085, sample_size=720
    ),
    "dso": _create_benchmark(
        "dso",
        Industry.HEALTHCARE,
...
def get_benchmark_sources() -> dict[str, Any]:
    ...
    return {
        ...
        "coverage": {"industries": len(INDUSTRY_BENCHMARKS), "ratios_per_industry": 8, "fiscal_year": 2025},
        "disclaimer": (
