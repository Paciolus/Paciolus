...
def _build_roll_forward(
    ...
    if additions is not None:
        ...
        story.append(
            Paragraph(
                create_leader_dots("Beginning Balance", _fmt_currency(beginning_cost)),
                styles["MemoLeader"],
            )
        )
        story.append(
            Paragraph(
                create_leader_dots("Additions During Period", f"+ {_fmt_currency(additions)}"),
                styles["MemoLeader"],
            )
        )
        if disposals:
            story.append(
                Paragraph(
                    create_leader_dots("Disposals During Period", f"- {_fmt_currency(disposals)}"),
                    styles["MemoLeader"],
                )
            )
    else:
    ...
    story.append(
        Paragraph(
            create_leader_dots("Ending Balance (per Register)", _fmt_currency(register_total_cost)),
            styles["MemoLeader"],
        )
    )
    ...
    if tb_ppe_gross is not None:
        story.append(
            Paragraph(
                create_leader_dots("Per Trial Balance (PP&amp;E gross)", _fmt_currency(tb_ppe_gross)),
                styles["MemoLeader"],
            )
        )
        ...
        story.append(
            Paragraph(
                create_leader_dots("Variance", _fmt_currency(cost_variance)),
                styles["MemoLeader"],
            )
        )
    ...
    if depr_expense is not None and additions is not None:
        ...
        story.append(
            Paragraph(
                create_leader_dots("Beginning Balance", _fmt_currency(beginning_accum)),
                styles["MemoLeader"],
            )
        )
        story.append(
            Paragraph(
                create_leader_dots("Depreciation Expense for Period", f"+ {_fmt_currency(depr_expense)}"),
                styles["MemoLeader"],
            )
        )
    ...
    story.append(
        Paragraph(
            create_leader_dots("Ending Balance (per Register)", _fmt_currency(register_total_accum_depr)),
            styles["MemoLeader"],
        )
    )
    ...
    if tb_accum_depr is not None:
        story.append(
            Paragraph(
                create_leader_dots("Per Trial Balance (Accum. Depr.)", _fmt_currency(tb_accum_depr)),
                styles["MemoLeader"],
            )
        )
        ...
        story.append(
            Paragraph(
                create_leader_dots("Variance", _fmt_currency(depr_variance)),
                styles["MemoLeader"],
            )
        )
    ...
    story.append(
        Paragraph(
            create_leader_dots("Net Book Value", _fmt_currency(nbv)),
            styles["MemoLeader"],
        )
    )
    ...
    if tb_ppe_gross is not None and tb_accum_depr is not None:
        ...
        if cost_var < 0.01 and depr_var < 0.01:
            ...
            parts = []
            if cost_var >= 0.01:
                parts.append(f"{_fmt_currency(cost_var)} in gross cost")
            if depr_var >= 0.01:
                parts.append(f"{_fmt_currency(depr_var)} in accumulated depreciation")
            story.append(
                Paragraph(
...
def _build_depreciation_rate_analysis(
    ...
    story.append(
        Paragraph(
            create_leader_dots("Gross PP&amp;E", _fmt_currency(register_total_cost)),
            styles["MemoLeader"],
        )
    )
    story.append(
        Paragraph(
            create_leader_dots("Depreciation Expense (Period)", _fmt_currency(depr_expense)),
            styles["MemoLeader"],
        )
    )
    story.append(
...
def _build_category_summary(
    ...
    for cat in cat_data:
        ...
        table_data.append(
            [
                ...
                Paragraph(_fmt_currency(cost), styles["MemoTableCell"]),
                Paragraph(_fmt_currency(depr), styles["MemoTableCell"]),
                Paragraph(_fmt_currency(nbv_cat), styles["MemoTableCell"]),
    ...
    table_data.append(
        [
            ...
            Paragraph(f"<b>{_fmt_currency(total_cost)}</b>", styles["MemoTableCell"]),
            Paragraph(f"<b>{_fmt_currency(total_depr)}</b>", styles["MemoTableCell"]),
            Paragraph(f"<b>{_fmt_currency(total_nbv)}</b>", styles["MemoTableCell"]),
    ...
    for fe in sorted_entries[:_MAX_DETAIL_ROWS]:
        ...
        table_data.append(
            [
                ...
                Paragraph(_fmt_currency(entry.get("cost", 0)), styles["MemoTableCell"]),
                Paragraph(_fmt_currency(entry.get("accumulated_depreciation", 0)), styles["MemoTableCell"]),
                Paragraph(_fmt_currency(excess), styles["MemoTableCell"]),
            ]
        )

    style_cmds = _standard_table_style() + [("ALIGN", (4, 0), (-1, -1), "RIGHT")]
    table = Table(
        table_data,
...
def _build_duplicate_assets_table(
    ...
    if total_dup_cost > 0:
        story.append(
            Paragraph(
                f"Potential duplicate capitalization: {_fmt_currency(total_dup_cost)} "
