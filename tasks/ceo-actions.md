...
- [ ] **`DB_TLS_OVERRIDE` expires 2026-05-09.** Break-glass in place because Neon's pooled connection has a `pg_stat_ssl` blind spot — the connection IS encrypted, but the in-database check can't see past the pooler. Proper fix is shipped as Sprint 673 (CODE-COMPLETE in `backend/database.py`); CEO must remove the env var on Render once startup logs show `tls=pooler-skip`.
