...
### Era 16: Launch-Readiness Era — Sprints 673–710 (2026-04-16 to 2026-04-22)
> Full archive: [`tasks/archive/sprints-673-710-launch-readiness-era.md`](archive/sprints-673-710-launch-readiness-era.md).
>
> Post-audit remediation batch (concentration overflow, tier entitlement wire-up, ISA 315 RMM matrix, ratio engine ROA/ROE/DuPont, ISA 570 going concern, JE Benford minimum, AR-aging date default, payroll/FA/AP/inventory test additions, MUS sampling Table A-1, revenue RT-09 split + RT-17, Enterprise PDF branding, Stripe trial-ending email, Coverage Sentinel non-prod ignore list, financial-report remediation). Anomaly framework hardening bundle (multi-currency defense-in-depth, engine contract layer, revenue/payroll/AR/FA/inventory alignment, CI guardrails). Security hardening follow-ups (Sprint 696 retroactive: passcode KDF upgrade + CSRF Sec-Fetch-Site + rate-limit fail-closed + HSTS gating + dev-user hygiene; Argon2id passcode + per-token + per-IP brute-force throttle; frontend passcode flow audit + public `/shared/[token]` page; legacy passcode cleanup; compliance docs refresh v2.7; auth-page autofill + wordmark). 2026-04-20 refactor pass (Sprint 710 retroactive: `shared/helpers.py` 1,075 → 150-line shim, 4 new cohesive backend modules, route thinning on org/billing, frontend TB upload unification). Design refresh (paper-grain texture, numeral-variant defaults, `<Blockquote>`, Merriweather kept as body serif, `<StandardsSpecimen>` replacing the homepage pill strip, `<ToolLedger>` 12-row grid replacing the one-card slideshow, demo signature moments: `<ScanlineOverlay>` + `<MechanicalGauge>` + `<MarginAnnotation>`, pricing brass Most Popular foil + editorial typography, `<EngravedStat>` monument, polish batch). Composite Risk + Account Risk Heatmap frontend wired. Public `/shared/[token]` share-receive page. **Tests: 8,028 backend + 1,887 frontend.**
