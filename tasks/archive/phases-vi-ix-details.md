# Archived Sprint Details: Phases VI-IX

> **Archived:** 2026-02-08 — Moved from todo.md to reduce context bloat.
...
- [x] Redirect `/` â†’ `/tools/trial-balance` temporarily (full homepage rebrand in Sprint 66)
...
- [x] Page orchestrates dual audit + comparison flow
  ...
  - Flow: upload both â†’ audit both via /audit/trial-balance â†’ compare â†’ display results
...
- [x] Frontend three-way UI: `+ Add Budget/Forecast` toggle, third FileDropZone, budget label input
  ...
  - Grid layout adjusts 2-col â†’ 3-col when budget enabled
...
- [x] **T1: Unbalanced Entries** — Groups by entry_id/reference, flags debitâ‰ credit, severity by difference
...
### Sprint 65: JE Testing — Statistical Tests + Benford Pre-Checks — COMPLETE âœ…
...
- [x] Minimum magnitude range: entries must span 2+ orders of magnitude ($10â†’$1,000+)
...
### Sprint 66: JE Testing — Frontend MVP + Platform Rebrand — COMPLETE âœ…
...
- [x] Tool Showcase Section: three tool cards with distinct icons, badges, descriptions
  - Trial Balance Diagnostics â†’ `/tools/trial-balance` (Headliner badge)
  - Multi-Period Comparison â†’ `/tools/multi-period` (Tool 2 badge)
  - Journal Entry Testing â†’ `/tools/journal-entry-testing` (Tool 3 badge)
...
- [x] SamplingPanel component (3-step: configure â†’ preview â†’ results)
...
- [x] Verify all three tool routes respect auth gating (verified users only)
  - 3-state rendering: guest â†’ Sign In CTA, authenticated unverified â†’ VerificationBanner + blocked, verified â†’ full access
...
- `frontend/package.json` (version 0.1.0 â†’ 0.60.0)
...
> **Selection criteria:** Value Ã— Leverage — prioritized features that reuse existing engines
...
> - Duplicate Payment Detector clones JE Testing pattern (column detection â†’ parse â†’ test battery â†’ score â†’ export)
...
- **FrontendExecutor:** All three follow established component patterns (Section/Card, upload â†’ results â†’ export)
...
- [x] Reduced `main.py` from 4,690 â†’ 63 lines (app + middleware + router registration)
...
- [x] Create `backend/financial_statement_builder.py` (~210 lines)
  ...
  - `build()` â†’ `FinancialStatements` dataclass
  - `_get_lead_sheet_balance(letter)` — 0.0 if missing
  - `_build_balance_sheet()` â†’ list[StatementLineItem] (A-K with sign conventions)
  - `_build_income_statement()` â†’ list[StatementLineItem] (L-O with sign conventions)
...
- [x] Add `generate_financial_statements_pdf()` to `pdf_generator.py` (+120 lines)
  ...
  - Balance verification seal: "âœ“ BALANCED" or "âš  OUT OF BALANCE ($X.XX)"
...
### Sprint 73: AP Testing — Backend Foundation + Tier 1 Tests — COMPLETE âœ…
> **Complexity:** 5/10 | **Agent Lead:** BackendCritic
...
- [x] Create `backend/ap_testing_engine.py` (~850 lines)
  ...
  - `detect_ap_columns(column_names)` â†’ `APColumnDetectionResult` (greedy assignment)
  - `APPayment` dataclass: single `amount` field (not debit/credit)
  - `parse_ap_payments(rows, detection)` â†’ `list[APPayment]`
...
- [x] **AP-T2: Missing Critical Fields** — vendor blankâ†’HIGH, amount==0â†’HIGH, date blankâ†’MEDIUM
...
- [x] **AP-T5: Payment Before Invoice** — severity: >30d HIGH, >7d MEDIUM, â‰¤7d LOW
...
- [x] **AP-T6: Fuzzy Duplicate Payments** — same vendor + same amount (Â±tolerance) + different dates within window
...
- [x] **AP-T10: High-Frequency Vendors** — same vendor â‰¥5 payments on one day (HIGH â‰¥10, MEDIUM â‰¥5)

#### Tier 3 Tests — Fraud Indicators (3 tests)
- [x] **AP-T11: Vendor Name Variations** — SequenceMatcher ratio â‰¥0.85 but names differ
...
- [x] Scoring calibration: clean_score â‰¤ moderate_score (comparative assertions)
...
- [x] Updated existing battery tests (5â†’13) and pipeline tests (5â†’13)
...
- [x] AP Testing section in Practice Settings (`/settings/practice`)
  ...
  - Key thresholds: Round Amount ($), Duplicate Date Window (days), Unusual Amount Sensitivity (Ïƒ), Keyword Sensitivity (%)
...
- [x] ReconciliationBridge component — standard bank rec workpaper format (bank â†’ adjusted bank, GL â†’ adjusted GL)
...
- [x] Update homepage tool showcase: 3 â†’ 5 tool cards
  ...
  - Updated hero copy: "Three integrated tools" â†’ "Five integrated tools"
  - Updated showcase heading: "Three Tools. One Platform." â†’ "Five Tools. One Platform."
...
- `frontend/package.json` (version 0.60.0 â†’ 0.70.0)
- `CLAUDE.md` (Phase VII â†’ COMPLETE, version 0.70.0, updated capabilities)
- `tasks/todo.md` (Sprint 80 â†’ COMPLETE)
...
- [x] `parse_uploaded_file(file_bytes, filename)` â†’ `tuple[list[str], list[dict]]` — CSV/Excel parsing
- [x] `parse_json_mapping(raw_json, log_label)` â†’ `Optional[dict[str, str]]` — JSON column mapping
- [x] `safe_download_filename(raw_name, suffix, ext)` â†’ `str` — sanitized timestamped filenames
...
- [x] Created `frontend/src/lib/downloadBlob.ts` — fetch â†’ blob â†’ anchor â†’ download pattern
...
- [x] Updated hero copy: "Five" â†’ "Six integrated tools"
- [x] Updated showcase heading: "Five Tools" â†’ "Six Tools"
...
> **Result:** ~588 lines added, ~1,003 lines removed â†’ ~415 lines net reduction
...
### Sprint 96: Phase IX Wrap — Navigation + Homepage + Regression âœ…
> **Complexity:** 2/10 | **Agent Lead:** QualityGuardian + FintechDesigner
> **Status:** COMPLETE
...
- [x] Updated hero copy: "Six integrated tools" â†’ "Seven integrated tools"
- [x] Updated showcase heading: "Six Tools. One Platform." â†’ "Seven Tools. One Platform."

#### CLAUDE.md Updates
- [x] Phase status: Phase IX COMPLETE (Sprint 96)
- [x] Version: 0.80.0 â†’ 0.90.0
...
- [x] Updated tool count references (6 â†’ 7)
- [x] Added Phase IX Overview sprint table
- [x] Next Priority: Phase X Planning

#### Version
- [x] `frontend/package.json` version: 0.70.0 â†’ 0.90.0
