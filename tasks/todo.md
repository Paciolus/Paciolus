...
- [2026-05-11] qa_warden orchestrator timeout fix — `DEFAULT_AGENT_TIMEOUT_S` (900s) was less than qa_warden's own internal subprocess budget (1200s pytest + 300s jest = 1500s worst case). Buffer eroded as backend test count grew: 2026-05-07 ran 817s/83s spare, 2026-05-08 ran 843s/57s spare, 2026-05-11 tipped to TIMEOUT (killed at 900s, agent did not run, nightly went RED). Added `qa_warden: 1700` to `AGENT_TIMEOUT_OVERRIDES` (same pattern as `coverage_sentinel`); 200s of margin over the agent's worst-case internal budget. File: `scripts/overnight/orchestrator.py`.
