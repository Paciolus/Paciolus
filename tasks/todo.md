...
- [2026-04-22] 9d8da04: gitignore `.claude/worktrees/`, `.hypothesis/`, `tests/evaluatingfiles/` — three untracked paths that recurred in `git status` across sessions. First and second are ephemeral tooling/cache state; third is ad-hoc CEO-validation TB fixtures (tb_hartwell.docx/.pdf + 4 CSVs — CSVs were already caught by the global `*.csv` rule, but the docx/pdf were not). Also cleaned the stray `.claude/worktrees/cool-morse` leftover from a prior isolation run.
