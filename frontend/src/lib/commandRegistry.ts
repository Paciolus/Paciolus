...
const TIER_ORDER: UserTier[] = ['free', 'solo', 'professional', 'enterprise']
...
function toolGuard(toolName: string): CommandGuard | undefined {
  // Free tools → no guard needed
  if (FREE_TOOLS.has(toolName)) return undefined
  // All other tools require at least Solo
  return { minTier: 'solo' }
}
