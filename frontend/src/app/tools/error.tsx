...
/**
 * Tools Error Boundary — catches errors in tool page components.
 *
 * Renders within the tools layout, so ToolNav + EngagementBanner
 * remain visible. Light-theme via semantic tokens.
 *
    ...
    <main className="min-h-screen bg-surface-page">
      <div className="pt-24 pb-16 px-6 max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-content-secondary text-center mb-8 leading-relaxed max-w-md">
          This diagnostic tool encountered an error. Your data has not been affected
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mb-6 p-4 w-full bg-surface-card rounded-lg border border-clay-200/30 text-left">
            <p className="text-xs font-mono text-clay-500 break-all">
              {error.message}
            </p>
            {error.digest && (
