...
import type { PayrollTestResult, PayrollTestTier, PayrollSeverity } from '@/types/payrollTesting'

const TIER_LABELS: Record<PayrollTestTier, string> = {
  structural: 'Structural',
  statistical: 'Statistical',
  advanced: 'Fraud Indicators',
}

const TIER_BADGE: Record<PayrollTestTier, string> = {
  structural: 'bg-obsidian-700 text-oatmeal-300',
  statistical: 'bg-sage-500/15 text-sage-400',
  advanced: 'bg-clay-500/10 text-clay-400',
}

const SEVERITY_BG: Record<PayrollSeverity, string> = {
...
function PayrollTestResultCard({ result }: { result: PayrollTestResult }) {
          ...
          >
            <div className="px-5 pb-4 border-t border-obsidian-600/20 pt-3">
              <p className="text-oatmeal-500 text-xs font-sans mb-2">
                Showing first {Math.min(result.flagged_entries.length, 5)} flagged employees:
              </p>
                      ...
                      <div className="min-w-0">
                        <p className="font-sans text-xs text-oatmeal-300 truncate">
                          ...
                          {fe.entry.employee_id && (
                            <span className="text-oatmeal-600 ml-2">#{fe.entry.employee_id}</span>
                          )}
                        </p>
              ...
              {result.flagged_entries.length > 5 && (
                <p className="text-oatmeal-600 text-xs font-sans mt-2 text-center">
                  + {result.flagged_entries.length - 5} more employees
                </p>
              )}
            </div>
          </motion.div>
...
export function PayrollTestResultGrid({ results }: PayrollTestResultGridProps) {
  ...
  return (
    <div className="space-y-6">
      {structural.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-oatmeal-300 mb-3">Structural Tests (PR-T1 to PR-T5)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {structural.map((r) => (
              <PayrollTestResultCard key={r.test_key} result={r} />
            ))}
          </div>
        </div>
      )}
      {statistical.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-oatmeal-300 mb-3">Statistical Tests (PR-T6 to PR-T8)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {statistical.map((r) => (
              <PayrollTestResultCard key={r.test_key} result={r} />
            ))}
      ...
      {advanced.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-oatmeal-300 mb-3">Fraud Indicators (PR-T9 to PR-T11)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {advanced.map((r) => (
              <PayrollTestResultCard key={r.test_key} result={r} />
            ))}
