...
import type { APCompositeScore, APRiskTier } from '@/types/apTesting'
import { RISK_TIER_COLORS, RISK_TIER_LABELS } from '@/types/apTesting'

const tierLeftBorder: Record<APRiskTier, string> = {
  low: 'border-l-4 border-l-sage-500',
  elevated: 'border-l-4 border-l-oatmeal-400',
  moderate: 'border-l-4 border-l-clay-400',
  high: 'border-l-4 border-l-clay-500',
  critical: 'border-l-4 border-l-clay-600',
}
...
export function APScoreCard({ score }: APScoreCardProps) {
  ...
  const leftBorder = tierLeftBorder[tier]
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-2xl bg-surface-card border border-theme shadow-theme-card ${leftBorder} p-8`}
    >
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
        ...
        <div className="flex-1 text-center md:text-left">
          <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${colors.bg} border ${colors.border} mb-3`}>
            <div className={`w-2 h-2 rounded-full ${tier === 'low' ? 'bg-sage-500' : tier === 'elevated' ? 'bg-oatmeal-400' : 'bg-clay-500'}`} />
          ...
          <p className="font-sans text-content-secondary text-sm mb-4">
            {score.tests_run} tests analyzed {score.total_entries.toLocaleString()} payments
          </p>
