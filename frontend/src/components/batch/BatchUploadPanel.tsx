...
function BatchUploadPanelContent({
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="font-serif text-2xl text-content-primary">{title}</h2>
        <p className="text-content-tertiary mt-1">{description}</p>
      </div>
      ...
      <motion.div
        ...
        className="flex items-start gap-3 p-4 bg-surface-card-secondary/30 rounded-lg border border-theme"
      >
        ...
        <div>
          <p className="text-sm text-content-secondary font-medium">Zero-Storage Processing</p>
  ...
  return (
    <BatchUploadProvider>
      <motion.div
        ...
        className={cx(
          'p-6 bg-surface-page rounded-2xl border border-theme',
          className
        )}
      >
