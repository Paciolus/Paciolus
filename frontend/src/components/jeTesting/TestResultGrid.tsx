...
import type { JETestResult, JETestTier, JESeverity } from '@/types/jeTesting'

const TIER_LABELS: Record<JETestTier, string> = {
  structural: 'Structural',
  statistical: 'Statistical',
  advanced: 'Advanced',
}

const TIER_BADGE: Record<JETestTier, string> = {
  structural: 'bg-oatmeal-100 text-oatmeal-700',
  statistical: 'bg-sage-50 text-sage-700',
  advanced: 'bg-clay-50 text-clay-700',
}

const SEVERITY_BG: Record<JESeverity, string> = {
  high: 'border-l-clay-500',
  medium: 'border-l-oatmeal-400',
  low: 'border-l-oatmeal-200',
}
...
function TestResultCard({ result }: { result: JETestResult }) {
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="font-sans text-xs text-content-tertiary line-clamp-2">{result.description}</p>
          </div>
        ...
        {expanded && hasFlags && (
          ...
          >
            <div className="px-5 pb-4 border-t border-theme-divider pt-3">
              <p className="text-content-tertiary text-xs font-sans mb-2">
                  ...
                  <div key={i} className="bg-surface-card-secondary rounded-lg p-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="font-sans text-xs text-content-tertiary mt-0.5">{fe.issue}</p>
                      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {statistical.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-content-primary mb-3">Statistical Tests (T6-T8)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {statistical.map((r) => (
              <TestResultCard key={r.test_key} result={r} />
            ))}
      ...
      {advanced.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-content-primary mb-3">Advanced Tests (T9-T18)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {advanced.map((r) => (
              <TestResultCard key={r.test_key} result={r} />
            ))}
