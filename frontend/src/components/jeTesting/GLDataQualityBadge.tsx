...
export function GLDataQualityBadge({ quality }: GLDataQualityBadgeProps) {
  ...
  return (
    ...
    >
      ...
      {/* Quality bar */}
      <div className="w-full h-2 bg-oatmeal-200 rounded-full overflow-hidden mb-4">
        <motion.div
        ...
        {Object.entries(quality.field_fill_rates)
          ...
          .map(([field, rate]) => (
            <div key={field} className="flex items-center justify-between">
              <span className="font-sans text-xs text-content-tertiary capitalize">
                {field.replace(/_/g, ' ')}
              </span>
      ...
      {quality.detected_issues.length > 0 && (
        <div className="mt-3 pt-3 border-t border-theme-divider">
          {quality.detected_issues.map((issue, i) => (
            <p key={i} className="font-sans text-xs text-content-tertiary mt-1">
      ...
      <p className="font-sans text-xs text-content-tertiary mt-3">
        {quality.total_rows.toLocaleString()} rows analyzed
      </p>
    </motion.div>
  )
}
