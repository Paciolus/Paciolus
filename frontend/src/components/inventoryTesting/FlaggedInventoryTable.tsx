...
import type { InvTestResult, FlaggedInventoryEntry, InvSeverity } from '@/types/inventoryTesting'
...
type SortField = 'item' | 'quantity' | 'value' | 'category' | 'severity' | 'test'
type SortDir = 'asc' | 'desc'

const SEVERITY_ORDER: Record<InvSeverity, number> = { high: 3, medium: 2, low: 1 }

function severityBadge(severity: InvSeverity) {
  const colors: Record<InvSeverity, string> = {
...
export function FlaggedInventoryTable({ results }: FlaggedInventoryTableProps) {
  ...
  const [severityFilter, setSeverityFilter] = useState<InvSeverity | 'all'>('all')
  ...
  const allFlagged = useMemo(() => {
    const entries: FlaggedInventoryEntry[] = []
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      items = items.filter(fe =>
        (fe.entry.item_id || '').toLowerCase().includes(q) ||
    ...
    items.sort((a, b) => {
      ...
      switch (sortField) {
        case 'severity':
          ...
          break
        case 'value':
          cmp = a.entry.extended_value - b.entry.extended_value
          break
        case 'quantity':
        ...
        case 'test':
          ...
          break
        case 'item':
          cmp = (a.entry.item_id || a.entry.description || '').localeCompare(b.entry.item_id || b.entry.description || '')
          break
        case 'category':
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 bg-surface-card border border-theme rounded-xl p-4 shadow-theme-card">
        <input
          type="text"
          placeholder="Search item ID, description, issue..."
          value={searchQuery}
        ...
        <select
          ...
          onChange={e => { setSeverityFilter(e.target.value as InvSeverity | 'all'); setPage(0) }}
          ...
          <thead>
            <tr className="border-b border-theme">
              {[
                ...
                { field: 'item' as SortField, label: 'Item' },
