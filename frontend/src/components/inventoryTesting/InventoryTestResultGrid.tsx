...
import type { InvTestResult, InvTestTier, InvSeverity } from '@/types/inventoryTesting'

const TIER_LABELS: Record<InvTestTier, string> = {
  structural: 'Structural',
  statistical: 'Statistical',
  advanced: 'Advanced',
}

const TIER_BADGE: Record<InvTestTier, string> = {
  structural: 'bg-oatmeal-100 text-oatmeal-700',
  statistical: 'bg-sage-50 text-sage-700',
  advanced: 'bg-clay-50 text-clay-700',
}

const SEVERITY_BG: Record<InvSeverity, string> = {
...
function InvTestResultCard({ result }: { result: InvTestResult }) {
...
export function InventoryTestResultGrid({ results }: InventoryTestResultGridProps) {
  ...
  return (
    <div className="space-y-6">
      {structural.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-content-primary mb-3">Structural Tests (IN-01 to IN-03)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {structural.map((r) => (
              <InvTestResultCard key={r.test_key} result={r} />
            ))}
          </div>
        </div>
      )}
      {statistical.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-content-primary mb-3">Statistical Tests (IN-04 to IN-07)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {statistical.map((r) => (
              <InvTestResultCard key={r.test_key} result={r} />
            ))}
      ...
      {advanced.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-content-primary mb-3">Advanced Tests (IN-08 to IN-09)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {advanced.map((r) => (
              <InvTestResultCard key={r.test_key} result={r} />
            ))}
