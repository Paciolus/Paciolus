/**
 * Dashboard tool icon registry (Sprint 751).
 *
 * Single source of truth for the SVG path data used by the dashboard's
 * Quick Launch grid + activity feed. Decouples icon paths from the page
 * so a path tweak doesn't force a page diff (which would otherwise drag
 * the entire 500-line dashboard JSX through review).
 *
 * Icon keys are constrained by `DashboardToolIconKey` from
 * `content/dashboard-tools.ts`. Adding a new icon requires extending
 * both the type union there and the `TOOL_ICON_PATHS` map here.
 */
import type { ReactElement } from 'react'
import type { DashboardToolIconKey } from '@/content/dashboard-tools'

const TOOL_ICON_PATHS: Record<DashboardToolIconKey, ReactElement> = {
...
export interface ToolIconProps {
  iconKey: DashboardToolIconKey | string | undefined
  className?: string
}

/**
 * Renders the SVG icon for a dashboard tool. Unknown keys fall back to
 * the cube icon (matches the legacy behavior of `getToolIcon`).
 */
export function ToolIcon({ iconKey, className = 'w-5 h-5' }: ToolIconProps) {
  const path = (iconKey && TOOL_ICON_PATHS[iconKey as DashboardToolIconKey]) ?? TOOL_ICON_PATHS.cube
  return (
