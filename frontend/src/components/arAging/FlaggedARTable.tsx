...
import type { ARTestResult, FlaggedAREntry, ARSeverity } from '@/types/arAging'
...
type SortField = 'customer' | 'amount' | 'aging' | 'severity' | 'test'
type SortDir = 'asc' | 'desc'

const SEVERITY_ORDER: Record<ARSeverity, number> = { high: 3, medium: 2, low: 1 }

function severityBadge(severity: ARSeverity) {
  const colors: Record<ARSeverity, string> = {
...
export function FlaggedARTable({ results }: FlaggedARTableProps) {
  ...
  const [severityFilter, setSeverityFilter] = useState<ARSeverity | 'all'>('all')
  ...
  const testKeys = useMemo(() => {
    ...
    for (const tr of results) {
      if (tr.entries_flagged > 0 && !tr.skipped) keys.add(tr.test_key)
    }
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      items = items.filter(fe =>
        (fe.entry.customer_name || '').toLowerCase().includes(q) ||
    ...
    items.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'test':
          ...
          break
        case 'customer':
          cmp = (a.entry.customer_name || a.entry.account_name || '').localeCompare(
            b.entry.customer_name || b.entry.account_name || ''
          )
          break
        case 'aging':
          cmp = (a.entry.aging_days ?? 0) - (b.entry.aging_days ?? 0)
          break
      }
  ...
  if (allFlagged.length === 0) {
    return (
      <div className="bg-surface-card-secondary border border-theme rounded-xl p-8 text-center">
        <p className="font-sans text-content-secondary">No flagged items found. All tests returned clean results.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 bg-surface-card-secondary border border-theme rounded-xl p-4">
        <input
          type="text"
          placeholder="Search customer, account, invoice..."
          value={searchQuery}
        ...
        <select
          ...
          onChange={e => { setSeverityFilter(e.target.value as ARSeverity | 'all'); setPage(0) }}
        ...
        <span className="font-sans text-xs text-content-tertiary">
          {filtered.length} of {allFlagged.length} items
        </span>
      </div>
          ...
          <thead>
            <tr className="border-b border-theme">
              {[
                ...
                { field: 'customer' as SortField, label: 'Customer / Account' },
                { field: 'amount' as SortField, label: 'Amount' },
                { field: 'aging' as SortField, label: 'Days' },
