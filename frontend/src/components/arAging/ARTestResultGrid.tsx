...
function ARTestResultCard({ result }: { result: ARTestResult }) {
  ...
  return (
    <motion.div
      ...
      className={`bg-obsidian-800/50 border border-obsidian-600/30 border-l-4 ${isSkipped ? 'border-l-obsidian-600 opacity-60' : SEVERITY_BG[result.severity]} rounded-xl overflow-hidden`}
    >
      <button
        onClick={() => !isSkipped && setExpanded(!expanded)}
        className={`w-full text-left p-5 ${isSkipped ? 'cursor-default' : 'hover:bg-obsidian-700/20'} transition-colors`}
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              {isSkipped && (
                <span className="px-2 py-0.5 rounded text-[10px] font-sans font-medium bg-obsidian-700 text-oatmeal-600 border border-obsidian-600">
                  Skipped
                </span>
              )}
            </div>
            <h4 className="font-serif text-sm text-oatmeal-200 mb-1">{result.test_name}</h4>
            <p className="font-sans text-xs text-oatmeal-500 line-clamp-2">
              {isSkipped ? result.skip_reason : result.description}
            </p>
          </div>
          <div className="text-right flex-shrink-0">
            {isSkipped ? (
              <span className="font-mono text-lg text-oatmeal-600">&mdash;</span>
            ) : hasFlags ? (
              <>
                      ...
                      <div className="min-w-0">
                        <p className="font-sans text-xs text-oatmeal-300 truncate">
                          ...
                          {fe.entry.aging_days != null && (
                            <span className="text-oatmeal-600 ml-2">{fe.entry.aging_days}d</span>
                          )}
                        </p>
...
export function ARTestResultGrid({ results }: ARTestResultGridProps) {
  ...
  return (
    <div className="space-y-6">
      {structural.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-oatmeal-300 mb-3">Structural Tests (AR-01 to AR-04)</h3>
      ...
      {statistical.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-oatmeal-300 mb-3">Statistical Tests (AR-05 to AR-09)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {statistical.map((r) => (
              <ARTestResultCard key={r.test_key} result={r} />
            ))}
      ...
      {advanced.length > 0 && (
        <div>
          <h3 className="font-serif text-sm text-oatmeal-300 mb-3">Advanced Tests (AR-10 to AR-11)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {advanced.map((r) => (
              <ARTestResultCard key={r.test_key} result={r} />
            ))}
