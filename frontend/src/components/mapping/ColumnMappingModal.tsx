...
export function ColumnMappingModal({
  ...
  const getConfidenceDisplay = (confidence: number) => {
    ...
    if (confidence >= 0.8) {
      ...
      return <span className="text-oatmeal-300 font-mono">{percentage}%</span>;
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        ...
        className="relative bg-obsidian-800 border border-obsidian-500 rounded-2xl shadow-2xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto"
      >
        {/* Header */}
        <div className="p-6 border-b border-obsidian-600">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 id="column-mapping-title" className="text-xl font-serif font-bold text-oatmeal-200">
          ...
          <p className="text-oatmeal-400 font-sans text-sm mt-3">
            We couldn&apos;t confidently identify all columns in <span className="text-oatmeal-200 font-mono">{filename}</span>.
        ...
        {columnDetection.detection_notes.length > 0 && (
          <div className="px-6 py-3 bg-obsidian-700/50 border-b border-obsidian-600">
            <p className="text-xs text-oatmeal-500 font-sans font-medium mb-1">Detection Notes:</p>
            <ul className="text-xs text-oatmeal-400 font-sans space-y-0.5">
        ...
        <div className="p-6 space-y-5">
          {/* Account Column */}
          <div>
            <label htmlFor="account-column" className="block text-sm font-sans font-medium text-oatmeal-200 mb-2">
              Account Name Column
              <span className="ml-2 text-xs text-oatmeal-500">
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-obsidian-700 border border-obsidian-500 rounded-xl text-oatmeal-200 font-sans focus:outline-none focus:ring-2 focus:ring-sage-500 focus:border-transparent"
            >
          ...
          {/* Debit Column */}
          <div>
            <label htmlFor="debit-column" className="block text-sm font-sans font-medium text-oatmeal-200 mb-2">
              Debit Column
              <span className="ml-2 text-xs text-oatmeal-500">
          ...
          {/* Credit Column */}
          <div>
            <label htmlFor="credit-column" className="block text-sm font-sans font-medium text-oatmeal-200 mb-2">
              Credit Column
              <span className="ml-2 text-xs text-oatmeal-500">
        ...
        {/* Footer */}
        <div className="p-6 border-t border-obsidian-600 flex gap-3 justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2.5 bg-obsidian-600 hover:bg-obsidian-500 text-oatmeal-200 font-sans font-medium rounded-xl border border-obsidian-500 transition-colors"
          >
            Cancel
          </button>
          <button
