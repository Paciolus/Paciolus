...
### Key Capabilities
- 17 core ratios (ROA / ROE with optional prior-period average-balance per Sprint 681) + 8 industry ratios across 6 benchmark industries + DuPont decomposition (math.isclose verification)
- 18 audit tools (Sprint 689 + Sprint 717 catalog reconciliation): JE (19 tests — Benford min 300 per Nigrini 2012), AP (14 tests incl. AP-T14 Invoice-Without-PO per ACFE 2024), Payroll (13 tests incl. PR-T12 Duplicate Names ghost-employee, PR-T13 Gross-to-Net Reconciliation), Revenue (18 tests — 14 core incl. split RT-09 cut-off / RT-09b prior-period-timing + RT-17 Contract Validity; 4 contract-aware ASC 606/IFRS 15), AR Aging (12 tests incl. AR-01b SL negative invoice), Fixed Assets (11 tests incl. FA-T11 Depreciation Recalculation), Inventory (10 tests incl. IN-T10 LCM/NRV), Bank Rec, Three-Way Match, Multi-Period TB, TB Diagnostics, Statistical Sampling (ISA 530 — AICPA Table A-1 expansion factors per Sprint 684), Segregation of Duties (689b), Intercompany Elimination (689c), W-2/W-3 Reconciliation (689d), Form 1099 Preparation (689e), Book-to-Tax Reconciliation (689f), Cash Flow Projector (689g). Multi-Currency Conversion is a side-car (defense-in-depth validation per Sprint 699), not a separate audit tool. **Single source of truth: `backend/tools_registry.py`.**
- TB Diagnostics: classification validator, lead sheets (A-Z), adjusting entries (approval-gated), lease/cutoff/going concern indicators (6+ indicators incl. cash flow + covenant breach — Sprint 685), going concern PDF section
- Composite Risk Scoring: ISA 315 **Appendix 1 RMM matrix** (Sprint 680 — not `max(IR, CR)`) — auditor-input workflow, inherent/control/fraud risk per account/assertion, combined with automated TB/testing data
- Financial Statements: Balance Sheet + Income Statement + Cash Flow (indirect, ASC 230/IAS 7)
- **18 memo PDFs + 3 report PDFs** (combined audit, financial statements, anomaly summary). Procedure standards cited in memos: PCAOB AS 2401/2501/2305/2310/2315, ISA 240/500/501/505/520/530/540/570/580. (PCAOB AS 1215 governs documentation/retention of all exports — see `backend/standards_registry.py`; it is **not** a procedure standard for any individual tool.) All memo PDFs support Enterprise-tier custom branding (logo + header/footer) via ContextVar propagation from the route (Sprint 679). Excel/CSV export available on all tiers above Free.
